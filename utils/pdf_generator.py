# ABOUTME: PDF report generation for sports dashboards using ReportLab.
# ABOUTME: Generates performance reports, injury reports, and player dossiers.
...
class SportsPDFGenerator:
    ...
    def create_player_dossier(
        ...
        """
        Generate a player dossier PDF including:
          - Visual header with cutout image (from CompositionMotor)
          - Season stats table
          - Generation timestamp and branding

        Args:
            player_id: Player identifier (used for cutout lookup and title)
            season: Season string (e.g. '2024-25')
            motor: CompositionMotor instance for header image generation

        Returns:
            BytesIO with the PDF content (seek position at 0)
        """
        try:
            buffer = BytesIO()
            doc = SimpleDocTemplate(buffer, pagesize=A4)
            ...
            # ── Header image ──
            try:
                header_img = motor.compose_dossier_header(player_id, season)
                img_buffer = BytesIO()
                header_img.save(img_buffer, format="PNG")
                img_buffer.seek(0)
                rl_image = Image(img_buffer, width=6 * inch, height=1.5 * inch)
                story.append(rl_image)
            except Exception as e:
                logger.warning(f"Could not embed dossier header image: {e}")

            story.append(Spacer(1, 16))

            # ── Title ──
            story.append(Paragraph(f"Player Dossier — {player_id}", self.title_style))
            story.append(Paragraph(f"Season: {season}", self.normal_style))
            story.append(Spacer(1, 12))

            # ── Generation timestamp ──
            ts = datetime.now().strftime("%d/%m/%Y %H:%M")
            story.append(Paragraph(f"Generated: {ts}", self.normal_style))
            story.append(Spacer(1, 20))

            # ── Stats placeholder (real stats injected by caller if available) ──
            story.append(Paragraph("Performance Summary", self.subtitle_style))
            summary_data = [
                ["Metric", "Value"],
                ["Player ID", player_id],
                ["Season", season],
                ["Report Type", "Individual Dossier"],
            ]
            table = self._create_table(summary_data, header_color=colors.darkblue)
            story.append(table)
            story.append(Spacer(1, 30))

            # ── Footer ──
            footer_style = ParagraphStyle(
                "DossierFooter", fontSize=8, alignment=1, textColor=colors.grey
            )
            story.append(Paragraph(
                "HK Premier League — Player Dossier generated by the Analytics Platform",
                footer_style,
            ))
            ...
            logger.error(f"Error generating player dossier for '{player_id}': {e}")
            buffer = BytesIO()
            doc = SimpleDocTemplate(buffer, pagesize=A4)
            doc.build([
                Paragraph("ERROR GENERATING DOSSIER", self.title_style),
                Spacer(1, 20),
                Paragraph(str(e), self.normal_style),
            ])
            buffer.seek(0)
            return buffer
