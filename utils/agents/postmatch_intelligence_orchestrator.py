# ABOUTME: Non-blocking orchestrator for postmatch-stage intelligence using the shared stage-runtime contract.
# ABOUTME: Persists postmatch artifacts, curates one current review candidate, and emits one UI-ready payload shape for callbacks and helpers.
...
from utils.agents.postmatch_signal_agent import curate_postmatch_signals, is_viable_postmatch_candidate
...
from utils.intelligence.session_memory import get_postmatch_session_memory, put_postmatch_session_memory
...
from utils.postmatch_stage.postmatch_intelligence import (
    ...
    _postmatch_source_checksum,
    build_postmatch_artifact_record,
    build_postmatch_signals_payload,
    build_postmatch_stage_analysis_payload,
    get_postmatch_intelligence_state,
)

logger = logging.getLogger(__name__)


def _persist_postmatch_artifact(
    artifact_type: str,
    ...
    *,
    ...
    """Persist one postmatch runtime artifact with graceful degradation."""
    record = build_postmatch_artifact_record(
        artifact_type,
        payload,
        player_id=player_id,
        match_id=match_id,
    ...
    try:
        ...
        logger.exception(
            "Postmatch orchestrator artifact persistence failed artifact=%s player_id=%s match_id=%s: %s",
            artifact_type,
            player_id,
            match_id,
            exc,
...
def _sync_postmatch_session_memory(
    *,
    player_id: str,
    match_id: str,
    ...
    """Persist postmatch session memory with graceful degradation."""
    ...
    return sync_session_memory_with_graceful_failure(
        persist_fn=lambda: put_postmatch_session_memory(
            player_id=player_id,
            match_id=match_id,
        ...
        logger=logger,
        log_message="Postmatch session memory persistence failed player_id=%s match_id=%s signal_id=%s: %s",
        log_args=(player_id, match_id, str((current_candidate or {}).get("signal_id") or "")),
        existing_memory=existing_memory,
    )


def orchestrate_postmatch_intelligence(
    payload: Dict[str, Any],
    *,
    registry: ArtifactRegistry | None = None,
    force_refresh: bool = False,
) -> Dict[str, Any]:
    """Orchestrate postmatch intelligence on the shared stage-runtime contract."""
    active_registry = registry or ArtifactRegistry()
    runtime_config = get_intelligence_runtime_config()
    state = get_postmatch_intelligence_state(payload, registry=active_registry)
    ...
    base_ready = all(artifact_type in runtime_artifacts for artifact_type in POSTMATCH_BASE_ARTIFACT_TYPES)
    scope = state.get("scope") or {}
    player_id = str(scope.get("player_id") or "")
    match_id = str(scope.get("match_id") or "")
    session_memory = get_postmatch_session_memory(player_id=player_id, match_id=match_id) or {}
    ...
    stored_analysis = (artifact_states.get(POSTMATCH_STAGE_ANALYSIS_ARTIFACT) or {}).get("artifact")
    ...
    reevaluation = decide_stage_reevaluation(
        ...
        primary_artifact_type=POSTMATCH_STAGE_ANALYSIS_ARTIFACT,
        dependency_fingerprints={},
        refresh_plan=refresh_plan,
        trigger_context={
            "force": force_refresh,
            "new_match_with_stats": bool(
                (payload.get("player_stats") or payload.get("minutes_played") is not None)
                and set(refresh_plan) & {POSTMATCH_MATCH_CONTEXT_ARTIFACT, POSTMATCH_PERFORMANCE_CONTEXT_ARTIFACT}
            ),
            "material_match_payload_changed": bool(
                set(refresh_plan)
                & set(POSTMATCH_BASE_ARTIFACT_TYPES + [POSTMATCH_STAGE_ANALYSIS_ARTIFACT])
            ),
    ...
    if (
        ...
        and stored_analysis
        and POSTMATCH_STAGE_ANALYSIS_ARTIFACT in runtime_artifacts
    ):
        stage_analysis = dict(stored_analysis.get("payload") or {})
        served_from["analysis"] = "artifact"
    else:
        stage_analysis = build_postmatch_stage_analysis_payload(
            payload,
            registry=active_registry,
            force_refresh=force_refresh or not runtime_config.get("persistence_enabled", True),
        )
        runtime_artifacts[POSTMATCH_STAGE_ANALYSIS_ARTIFACT] = {"payload": stage_analysis}
        persisted_artifacts.append(POSTMATCH_STAGE_ANALYSIS_ARTIFACT)
        served_from["analysis"] = str(((stage_analysis.get("debug") or {}).get("analysis_source")) or "inline")

    stored_signals = (artifact_states.get(POSTMATCH_SIGNALS_ARTIFACT) or {}).get("artifact")
    if stored_signals and POSTMATCH_SIGNALS_ARTIFACT in runtime_artifacts:
        signals_payload = dict(stored_signals.get("payload") or {})
        served_from["signals"] = "artifact"
    else:
        signals_payload = build_postmatch_signals_payload(stage_analysis)
        persisted_signal = _persist_postmatch_artifact(
            POSTMATCH_SIGNALS_ARTIFACT,
            signals_payload,
            player_id=player_id,
            match_id=match_id,
            base_payloads=state.get("base_payloads") or {},
            dependency_fingerprint_map={POSTMATCH_STAGE_ANALYSIS_ARTIFACT: str((artifact_states.get(POSTMATCH_STAGE_ANALYSIS_ARTIFACT) or {}).get("expected_fingerprint") or "")},
            registry=active_registry,
            runtime_config=runtime_config,
        )
        runtime_artifacts[POSTMATCH_SIGNALS_ARTIFACT] = persisted_signal["artifact"]
        if persisted_signal["persisted"]:
            persisted_artifacts.append(POSTMATCH_SIGNALS_ARTIFACT)
        elif persisted_signal["failure"]:
            write_failures.append({"target": POSTMATCH_SIGNALS_ARTIFACT, "error": persisted_signal["failure"]})
        served_from["signals"] = "inline"

    stored_overlay = (artifact_states.get(POSTMATCH_OVERLAY_CANDIDATES_ARTIFACT) or {}).get("artifact")
    if stored_overlay and POSTMATCH_OVERLAY_CANDIDATES_ARTIFACT in runtime_artifacts and not force_refresh:
        overlay_payload = dict(stored_overlay.get("payload") or {})
        overlay_surface = resolve_stage_overlay_surface("postmatch", overlay_payload)
        served_from["overlay"] = "artifact"
    else:
        curation = curate_postmatch_signals(
        ...
        persisted_overlay = _persist_postmatch_artifact(
            POSTMATCH_OVERLAY_CANDIDATES_ARTIFACT,
            overlay_payload,
            player_id=player_id,
            match_id=match_id,
            base_payloads=state.get("base_payloads") or {},
            dependency_fingerprint_map={POSTMATCH_SIGNALS_ARTIFACT: str((artifact_states.get(POSTMATCH_SIGNALS_ARTIFACT) or {}).get("expected_fingerprint") or "")},
            registry=active_registry,
            runtime_config=runtime_config,
        )
        runtime_artifacts[POSTMATCH_OVERLAY_CANDIDATES_ARTIFACT] = persisted_overlay["artifact"]
        if persisted_overlay["persisted"]:
            persisted_artifacts.append(POSTMATCH_OVERLAY_CANDIDATES_ARTIFACT)
        elif persisted_overlay["failure"]:
            write_failures.append({"target": POSTMATCH_OVERLAY_CANDIDATES_ARTIFACT, "error": persisted_overlay["failure"]})

    current_candidate = dict((overlay_surface or {}).get("primary_candidate") or {})
    if current_candidate and is_viable_postmatch_candidate(current_candidate):
        memory_result = _sync_postmatch_session_memory(
            player_id=player_id,
            match_id=match_id,
